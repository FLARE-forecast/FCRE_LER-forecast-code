...
run_config$sim_name <- paste0("ler_", tolower(mod), "_test3_", e, "ens_v1")
...
restart_file <- list.files(config$file_path$forecast_output_directory, pattern = c(run_config$sim_name), full.names = TRUE)
restart_file <- restart_file[grepl("*.nc", restart_file)]

file.copy(restart_file, gsub(".nc", "_restart.nc", restart_file))

restart_file <- gsub(".nc", "_restart.nc", restart_file)

restart_date <- get_restart_date(restart_file)

# unlink(config$file_path$execute_directory, recursive = TRUE)

config$run_config$restart_file <- basename(restart_file)
if(restart_date$status == 0) {
  config$run_config$start_datetime <- paste(restart_date$start_date, "00:00:00")
}
...
met_out <- list(filenames = list.files(config$file_path$execute_directory, pattern = "met_ens", full.names = T),
                historical_met_error = FALSE)

inflow_outflow_files <- list(inflow_file_name = list.files(config$file_path$execute_directory, pattern = "inflow1_ens", full.names = T),
                             outflow_file_name = list.files(config$file_path$execute_directory, pattern = "outflow1_ens", full.names = T))
