...
models <- c( "GLM", "Simstrat", "GOTM")
#models <- models[3]

for(mod in models) {
  configure_run_file <- paste0("configure_run_test1_", tolower(mod), ".yml")
  run_config <- yaml::read_yaml(file.path(lake_directory,"configuration",config_set_name,configure_run_file))
  run_config$sim_name <- paste0("ler_", tolower(mod), "_test3_noda")
  ...
  config$run_config
  config$da_setup$ensemble_size <- 30
  ...
  met_out <- FLAREr::generate_met_files(obs_met_file = file.path(config$file_path$qaqc_data_directory, paste0("observed-met_",config$location$site_id,".nc")),
                                        ...
                                        config = config)
  # lapply(met_out$filenames, function(x) {
  #   tst <- read.csv(x)
  #   print(tst[nrow(tst), 1:2])
  # })
  ...
  # lapply(inflow_outflow_files$inflow_file_names, function(x) {
  #   tst <- read.csv(x)
  #   print(tst[nrow(tst), 1:2])
  # })
