...
for(j in 1:length(sites)){
  ...
  config$model_settings$model <- models[1]
  ...
  for(i in time_start_index:length(forecast_start_dates)){
    ...
    if(i > 1){
      ...
      target <- readr::read_csv(cleaned_observations_file_long, show_col_types = FALSE) %>%
        dplyr::filter(hour == 0) %>%
        dplyr::group_by(date, hour, depth, variable) %>%
        dplyr::summarize(value = mean(value, na.rm = TRUE), .groups = "drop") %>%
        dplyr::select(date, hour, depth, value, variable)

      message("   creating climatology forecast")
      if(run_clim_null){

        target_clim <- target %>%
          dplyr::filter(date < as_date(config$run_config$forecast_start_datetime),
                 variable == "temperature") %>%
          dplyr::mutate(doy = yday(date)) %>%
          dplyr::group_by(doy, depth) %>%
          dplyr::summarise(temp_clim = mean(value, na.rm = TRUE),
                    temp_sd = sd(value, na.rm = TRUE), .groups = "drop") %>%
          dplyr::mutate(temp_sd = mean(temp_sd, na.rm = TRUE)) %>%
          dplyr::mutate(temp_clim = ifelse((is.nan(temp_clim)), NA, temp_clim)) %>%
          na.omit()

        clim_forecast <- target_clim %>%
          dplyr::mutate(doy = as.integer(doy)) %>%
          dplyr::filter(doy %in% forecast_doy) %>%
          dplyr::mutate(time = as_date((doy-1), origin = paste("2021", "01", "01", sep = "-"))) %>%
          dplyr::select(time, depth , temp_clim, temp_sd) %>%
          dplyr::rename(mean = temp_clim,
                 sd = temp_sd) %>%
          tidyr::pivot_longer(c("mean", "sd"),names_to = "statistic", values_to = "temperature") %>%
          dplyr::mutate(forecast = ifelse(time >= as_date(config$run_config$forecast_start_datetime), 1, 0)) %>%
          dplyr::select(time, depth, statistic, forecast, temperature) %>%
          dplyr::arrange(depth, time, statistic)
      ...
      if(run_persistence_null){
        ...
        for(k in 1:length(depths)){
          ...
          most_recent_obs_depth <- dplyr::most_recent_obs %>%
            dplyr::filter(depth == depths[k])
          ...
          depth_tibble <- dplyr::bind_cols(time = dates,depth_tibble) %>%
            tidyr::pivot_longer(cols = -time, names_to = "ensemble", values_to = "temperature") %>%
            dplyr::mutate(forecast = 1,
                   depth = depths[k]) %>%
            dplyr::select(time, depth, ensemble, forecast, temperature)
